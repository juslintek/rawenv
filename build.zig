...
[0m[0m
   // Per-service install→start→verify→stop→cleanup lifecycle E2E (E2E-107):[0m[0m
   // node, redis, meilisearch, postgres, bun, php, mariadb, python — each[0m[0m
   // exercised individually with no-side-effect teardown invariants.[0m[0m
   const integration_svc_lifecycle = b.addTest(.{[0m[0m
       .root_module = b.createModule(.{[0m[0m
           .root_source_file = b.path("tests/integration/service_lifecycle_e2e_test.zig"),[0m[0m
           .target = target,[0m[0m
           .optimize = optimize,[0m[0m
       }),[0m[0m
   });[0m[0m
   integration_svc_lifecycle.root_module.link_libc = true;[0m[0m
   const run_svc_lifecycle_test = b.addRunArtifact(integration_svc_lifecycle);[0m[0m
   run_svc_lifecycle_test.step.dependOn(b.getInstallStep());[0m[0m
   run_svc_lifecycle_test.setEnvironmentVariable("RAWENV_BIN", b.getInstallPath(.bin, "rawenv"));[0m[0m
   integration_step.dependOn(&run_svc_lifecycle_test.step);[0m[0m
[0m[0m
   // Multi-project isolation E2E (two postgres projects don't share data;[0m[0m
   // each destroys independently; shared store binaries are never removed).[0m[0m
   const integration_isolation = b.addTest(.{[0m[0m
       .root_module = b.createModule(.{[0m[0m
           .root_source_file = b.path("tests/integration/multi_project_isolation_test.zig"),[0m[0m
           .target = target,[0m[0m
           .optimize = optimize,[0m[0m
       }),[0m[0m
   });[0m[0m
   integration_isolation.root_module.link_libc = true;[0m[0m
   const run_isolation_test = b.addRunArtifact(integration_isolation);[0m[0m
   run_isolation_test.step.dependOn(b.getInstallStep());[0m[0m
   run_isolation_test.setEnvironmentVariable("RAWENV_BIN", b.getInstallPath(.bin, "rawenv"));[0m[0m
   integration_step.dependOn(&run_isolation_test.step);[0m[0m
[0m[0m
...
[0m[0m
   // Compose import E2E (real-world fixtures: Sail quoted images, azure-sql-edge[0m[0m
   // → mssql, depends_on → connections, port-mapping host ports).[0m[0m
   const integration_compose_import = b.addTest(.{[0m[0m
       .root_module = b.createModule(.{[0m[0m
           .root_source_file = b.path("tests/integration/compose_import_e2e_test.zig"),[0m[0m
           .target = target,[0m[0m
           .optimize = optimize,[0m[0m
       }),[0m[0m
   });[0m[0m
   integration_compose_import.root_module.link_libc = true;[0m[0m
   const run_compose_import_test = b.addRunArtifact(integration_compose_import);[0m[0m
   run_compose_import_test.step.dependOn(b.getInstallStep());[0m[0m
   run_compose_import_test.setEnvironmentVariable("RAWENV_BIN", b.getInstallPath(.bin, "rawenv"));[0m[0m
   integration_step.dependOn(&run_compose_import_test.step);[0m[0m
[0m[0m
