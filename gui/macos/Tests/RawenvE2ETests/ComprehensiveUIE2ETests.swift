...
struct ComprehensiveUIE2ETests {
    ...
    private func setValue(_ e: AXUIElement, _ v: String) {
    ...
    private func parent(_ e: AXUIElement) -> AXUIElement? {
        guard let p = attr(e, kAXParentAttribute),
            CFGetTypeID(p) == AXUIElementGetTypeID()
        else { return nil }
        // AX value verified to be an AXUIElement via CFGetTypeID above.
    ...
    private func cyclePicker(_ app: AXUIElement, _ window: AXUIElement, _ id: String) {
        ...
        press(picker)
        Thread.sleep(forTimeInterval: 0.2)  // close
    ...
    @Test func fullFlowEveryControlAndOption() throws {
        ...
        guard let window = windows(app).first else {
        ...
        for svc in findAllByPrefix(window, "service_") {
            press(svc)
        ...
        for btn in findAllByPrefix(window, "project_setup_btn_") {
            press(btn)
            Thread.sleep(forTimeInterval: 0.15)
        }
        ...
        for mode in ["conn_local_", "conn_proxy_", "conn_remote_"] {
            for e in findAllByPrefix(window, mode) {
                press(e)
                Thread.sleep(forTimeInterval: 0.15)
            }
        }
